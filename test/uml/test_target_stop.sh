...
ssh $source_uml_host "zumastor define target $vol $target_uml_host:$target_port -p 1" || { echo FAIL; exit 1; }
...
while [[ $count -lt $TOTAL ]]; do
	ssh $target_uml_host "/etc/init.d/zumastor stop" || { echo FAIL; exit 1; }
	...
	ssh $target_uml_host "/etc/init.d/zumastor start" || { echo FAIL; exit 1; }
	sleep 30
