...
struct change_list {
  struct list_head list;
  le_u64 changed_chunk;
};

struct change_list change_list_head;

/* snapshot1 is the older (temporal) snapshot compared to snapshot2 */
static void gen_changelist_leaf(struct eleaf *leaf, int snapshot1, int snapshot2, int * num_chunks) {
        struct exception *p;
	int i;
	u64 tag1 = 1LL << snapshot1, tag2 = 1LL << snapshot2;
	
	for (i = 0; i < leaf->count; i++) 
	  for (p = emap(leaf, i); p < emap(leaf, i+1); p++) { 
	    if( !(((p->share & tag2) == tag2) && ((p->share & tag1) == tag1))) {
	      struct change_list * newchunk = (struct change_list *)malloc(sizeof(struct change_list));
	      newchunk->changed_chunk = leaf->base_chunk + leaf->map[i].rchunk;
	      list_add(&(newchunk->list), &(change_list_head.list));
	      (*num_chunks)++;
	    }	
	  }	       
}

static void gen_changelist_subtree(struct superblock *sb, struct enode *node, int levels, int snapshot1, int snapshot2, int * num_chunks)
{
	int i;
	...
	for (i = 0; i < node->count; i++) {
		...
		if (levels)
			gen_changelist_subtree(sb, buffer2node(buffer), levels - 1, snapshot1, snapshot2, num_chunks);
		else {
			gen_changelist_leaf(buffer2leaf(buffer), snapshot1, snapshot2, num_chunks);
		}
		brelse(buffer);
	}
}

static void gen_changelist_tree(struct superblock *sb, int snapshot1, int snapshot2, int * num_chunks)
{
	struct buffer *buffer = snapread(sb, sb->image.etree_root);
	gen_changelist_subtree(sb, buffer2node(buffer), sb->image.etree_levels - 1, snapshot1, snapshot2, num_chunks);
	brelse(buffer);
}
...
int incoming(struct superblock *sb, struct client *client)
{
	...
	switch (message.head.code) {
		...
		{
		        int num_chunks = 0, change_fd, len = -1;
			int snap1 = ((struct generate_changelist *)message.body)->snap1;
		        int snap2 = ((struct generate_changelist *)message.body)->snap2;	
			struct list_head *pos, *safe;
			char * filename;

			/*
			int change_fd, bunksize;
			char bunk[512];
		       
			if ( (change_fd = recv_fd(sock, bunk, &bunksize)) < 0) {
			  trace_on(warn("unable to get file descriptor for changelist"));
			...
			*/
			...
			/* hack */
			if (read(sock, &len, sizeof(len)) < 0 || len < 1) {
			  trace_on(warn("failed reading filename length from client\n"));
			  break;
			}

			filename = (char *)malloc(len);
			if (read(sock, filename, len) < len) {
			  trace_on(warn("failed reading filename from client\n"));
			  break;
			}
			
			if ((change_fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC)) < 0) {
			  trace_on(warn("unable to open file %s\n", filename));
			  break;
			}
			/* done with hack */

		        trace_on(printf("changelist filename: %s snap1 %d and snap2 %d\n", filename, snap1, snap2));
			assert(list_empty(&change_list_head.list)); 
			free(filename);
			    
			gen_changelist_tree(sb,snap1,snap2, &num_chunks);
			
			/* do some writev stuff here */
			write(change_fd, "rln", 3);
			write(change_fd, &num_chunks, sizeof(num_chunks));
			write(change_fd, &(sb->image.chunksize_bits), sizeof(sb->image.chunksize_bits));

			/* this could really use some writev-like stuff */
			list_for_each(pos, &(change_list_head.list)) {
			  struct change_list * diff_chunk = list_entry(pos, struct change_list, list);
			  write(change_fd, &diff_chunk->changed_chunk, sizeof(diff_chunk->changed_chunk));
			  trace(printf("chunk %Lu added to changelist\n", diff_chunk->changed_chunk));
			}

			close(change_fd);
		        outbead(sock, REPLY_GENERATE_CHANGE_LIST, struct { });
			
			/* clean up the changelist linked list */
			struct change_list * free_me;
			list_for_each_safe(pos, safe, &(change_list_head.list)) {
			  free_me = list_entry(pos, struct change_list, list);
			  list_del(pos);
			  free(free_me);
			}
...
int snap_server(struct superblock *sb, const char *sockname, int port)
{
	...
	/* for the changelist linked list */
	INIT_LIST_HEAD(&(change_list_head.list));
