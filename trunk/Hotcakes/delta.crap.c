...
int delta_chunk_helper(int (*func) (xd3_stream *), 
	...
	*output_size = 0;
	...
	while(ret != 0) {
		...
		case XD3_OUTPUT:
			/* write data */
			printf("current output_size %d and avail_out %d max_size %d\n", *output_size, stream.avail_out, max_size);
			...
			*output_size = *output_size + stream.avail_out;
			printf("current output_size %d and avail_out %d\n", *output_size, stream.avail_out);
			xd3_consume_output(&stream);
    			continue;
  		case XD3_GETSRCBLK:
			printf("crap!\n");
	...
	printf("Exiting delta_chunk_helper\n");
	fflush(stdin);
	return ret;
...
error:
	...
	xd3_free_stream(&stream);
	printf("%s", err_msg);
	printf("Exiting delta_chunk_helper error\n");
	return ret;
}
